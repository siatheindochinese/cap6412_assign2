...
if 'opt_openvqa_result.json' not in os.listdir('precomputed_wgv'):
...
template = 'Question: Which country is the image from? Short answer:'
if 'opt_openvqa_result.json' not in os.listdir('precomputed_wgv'):
	text_processed = txt_processors["eval"](template)

###########################
# Open-ended VQA Pipeline #
###########################
result = []
if 'opt_openvqa_result.json' not in os.listdir('precomputed_wgv'):
	for i in range(len(imgpth)):
		...
		print(ans)
		result.append(ans[0])
	with open('precomputed_wgv/opt_openvqa_result.json','w') as f:
		json.dump(result, f)
		
result = json.load(open('precomputed_wgv/opt_openvqa_result.json'))
