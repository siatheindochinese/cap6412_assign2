...
if 't5_closedvqa_result.json' not in os.listdir('precomputed_wgv'):
...
if 't5_closedvqa_result.json' not in os.listdir('precomputed_wgv'):
	for i in range(len(imgpth)):
		...
		print(ans)
		result.append(ans[0])
	with open('precomputed_wgv/t5_closedvqa_result.json','w') as f:
		json.dump(result, f)
		
result = json.load(open('precomputed_wgv/t5_closedvqa_result.json'))
